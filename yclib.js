...
//对参数字符串的编码   针对get请求 person.action?name=&xxx&xxx&age=20   name  张三  age
	...
	function getRequestObject(url,options){
		...
		if(window.XMLHttpRequest){
			 ...
			 req= new window.ActiveXObject('Microsoft.XMLHTTP');//ie7以下浏览器
		}
		...
		req.onreadystatechange =function(){
			switch(req.readyState){//(0,1,2,3,4)
				case 1:
				//请求初始化时
				...
				try{
					if(req.status && req.status == 200){
						...
						//通过上述的正则，可以捕获到分号之前 text/html  text/json  text/application   text/plain
						switch(mimeType){
							...
							case 'text/json':
								...
								if(options.jsonResponseListener){
									...
									//options.jsonResponseListener.allpy(req,[json]);一次方法的借调
								}
								break;
							case 'text/xml':
